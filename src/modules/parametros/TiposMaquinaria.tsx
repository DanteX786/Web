...
import { Wrench, Plus, Pencil, Trash2, X, Search, AlertTriangle } from 'lucide-react';
...
const seedMaquinas: TipoMaquina[] = [
  { id: "MAQ-001", nombre: "Plana", desc: "Costura recta industrial" },
  { id: "MAQ-002", nombre: "Fileteadora", desc: "Acabado de bordes" },
  { id: "MAQ-003", nombre: "Collaretera", desc: "Costuras elásticas" },
];
...
export default function TiposMaquinaria({ dark }: { dark: boolean }) {
  const [data, setData] = useState<TipoMaquina[]>(seedMaquinas);
  const [search, setSearch] = useState("");
  const [modal, setModal] = useState<"add" | "edit" | null>(null);
  const [editing, setEditing] = useState<TipoMaquina | null>(null);
  ...
  function openAdd() {
    const nextCode = nextId("MAQ", data.map(d => d.id));
    setForm({ id: nextCode, nombre: "", desc: "" });
    setEditing(null);
    setModal("add");
  }

  function openEdit(item: TipoMaquina) {
  ...
  function save() {
    ...
    if (modal === "add") {
      ...
      toast.success("Tipo de máquina creado");
    } else if (editing) {
      setData(data.map(d => d.id === editing.id ? { ...d, nombre: form.nombre, desc: form.desc } : d));
      toast.success("Tipo de máquina actualizado");
    }
    setModal(null);
  }
  ...
  return (
    <div style={{ padding: 24, fontFamily: "Montserrat, sans-serif" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
        <div>
          ...
          <p style={{ fontSize: 13, color: subtle, marginTop: 2, margin: 0, fontFamily: "Montserrat, sans-serif" }}>
            Gestión de tipos de máquina registrados
          </p>
        </div>
        ...
        <table style={{ width: "100%", borderCollapse: "collapse", fontFamily: "Montserrat, sans-serif" }}>
          <thead>
            <tr style={{ background: dark ? "#252525" : "#FAFAFA", borderBottom: `2px solid ${GOLD}` }}>
              {["ID Tipo", "Nombre de la máquina", "Descripción", "Acciones"].map(h => (
          ...
          <div style={{ width: "100%", maxWidth: 520, borderRadius: 16, background: cardBg, border: `1px solid ${GOLD}40`, boxShadow: "0 20px 60px rgba(0,0,0,0.2)", overflow: "hidden" }}>
            ...
            <div style={{ padding: "16px 24px", borderBottom: `1px solid ${GOLD}25`, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <span style={{ fontSize: 16, fontWeight: 700, color: fg, fontFamily: "Montserrat, sans-serif" }}>
                {modal === "add" ? "Agregar Tipo de Máquina" : "Editar Tipo de Máquina"}
              </span>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
                <div>
                  <label style={{ display: "block", fontSize: 11, fontWeight: 700, textTransform: "uppercase", color: subtle, marginBottom: 6 }}>
                    {modal === "add" ? "Código (Auto)" : "Código de Máquina"}
                  </label>
                  <input
      ...
      {deleting && (
        <ConfirmDelete dark={dark} message="Tipo de máquina eliminado" onCancel={() => setDeleting(null)} onConfirm={() => confirmDelete(deleting)} />
      )}
    </div>
  );
}